...
from eval.sae_intervention import get_model_output_no_nnsight #TODO: Remove nnsight
...
def CE_for_orig_and_zero_ablation(esm_model, nnsight_model, tokenized_batches,
    ...
    for batch_tokens, batch_attn_mask in tqdm(tokenized_batches):
        ...
        orig_logits, orig_hidden = get_model_output_no_nnsight(
            esm_model, batch_tokens, batch_attn_mask, hidden_layer_idx
        )

        zero_logits, _ = get_model_output_no_nnsight(
...
def get_loss_recovery_fn(
    ...
    device: str,
    batch_size: int = 8,
    corrupt: bool = False #TODO: Remove this
) -> callable:
    ...
    nnsight_model = None #TODO: Remove nnsight
